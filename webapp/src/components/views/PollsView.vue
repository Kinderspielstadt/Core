  ...
  async (e) => {
    ...
    if (e.key === 'Enter' && option.value) {
      ...
      try {
        success.value = false;
        error.value = '';
        await PollsService.addPollOption({
          accountNumber: accountId.value,
          option: option.value,
        });
        accountId.value = '';
        success.value = true;
        ...
        if (e.status == 404) {
          ...
          e.status == 400 &&
          e.data?.data.accountNumber.code == 'validation_not_unique'
        ) {
          accountId.value = '';
...
<template>
  <AtomHeroText>
    <div v-if="!option" class="flex flex-col gap-4">
